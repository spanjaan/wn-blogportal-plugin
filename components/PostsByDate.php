...
class PostsByDate extends Posts
{
    ...
    protected function loadDate()
    {
        ...
        // Check if it's a year and month format (YYYY-MM or YYYY-M)
        if (count($parts) === 2 && strlen($parts[0]) === 4 && strlen($parts[1]) >= 1 && strlen($parts[1]) <= 2) {
            $year = $parts[0];
            $month = ltrim($parts[1], '0'); // Remove leading zeros
            $day = null;
            $type = 'month';
        }
        // Check if it's a year and month and day format (YYYY-MM-DD)
        elseif (count($parts) === 3 && strlen($parts[0]) === 4 && strlen($parts[1]) >= 1 && strlen($parts[1]) <= 2 && strlen($parts[2]) >= 1 && strlen($parts[2]) <= 2) {
            $year = $parts[0];
            $month = ltrim($parts[1], '0'); // Remove leading zeros
            $day = ltrim($parts[2], '0');   // Remove leading zeros
            $type = 'day';
        } 
